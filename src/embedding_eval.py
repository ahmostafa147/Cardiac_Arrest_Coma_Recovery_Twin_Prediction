"""
embedding_eval.py — embedding evaluation.
...
from constants import CLASS_NAMES, DISPLAY_ORDER, DISPLAY_NAMES, N_CLASSES
from config import prep, embeddings, FIG_EVAL as FIG_DIR, OUT

from config import RUN_TAG

# ── Load ────────────────────────────────────────────────────────────
print(f"=== Loading (RUN_TAG={RUN_TAG}) ===")
train_prep = np.load(prep('train'), allow_pickle=True)
test_prep  = np.load(prep('test'),  allow_pickle=True)
X_train_emb = np.load(embeddings('train'))['embedding']
X_test_emb  = np.load(embeddings('test'))['embedding']
...
def compute_infonce(emb, labels, n_samples=2048, temperature=1.0, n_runs=10):
    # a ref and a neg set are drawn without replacement, so the cohort has to
    # supply 2 * n_samples rows; clamp for cohorts smaller than that
    n_samples = min(n_samples, len(emb) // 2)
    losses = []
...
def make_cb(seed=42, **kw):
    return CatBoostClassifier(iterations=500, depth=6, learning_rate=0.05,
                              ...
                              train_dir=str(OUT / 'catboost_info'), **kw)
...
plt.tight_layout()
plt.savefig(f'{FIG_DIR}/cebra_t1_confusion_{RUN_TAG}.png', dpi=150, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(f'{FIG_DIR}/cebra_t2_cpc_{RUN_TAG}.png', dpi=150, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(f'{FIG_DIR}/cebra_centroid_distances_{RUN_TAG}.png', dpi=150, bbox_inches='tight')
plt.close()
